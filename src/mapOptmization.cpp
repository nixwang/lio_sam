...
class mapOptimization : public ParamServer {
  typedef message_filters::sync_policies::ApproximateTime<lio_sam::cloud_info,
...
public:
  ...
  std::unique_ptr<message_filters::Subscriber<lio_sam::cloud_info>> subCloudInfo;
  std::unique_ptr<message_filters::Subscriber<sensor_msgs::PointCloud2>> subLaserCloudOriginal;
  ...
  vector<pcl::PointCloud<PointXYZIRT>::Ptr> laserCloudKeyFrames;
  ...
  pcl::PointCloud<PointXYZIRT>::Ptr laserCloudLast;
  ...
  mapOptimization() {
    ...
    subCloudInfo.reset(
        new message_filters::Subscriber<lio_sam::cloud_info>(nh, "lio_sam/feature/cloud_info", 1));
    subLaserCloudOriginal.reset(
        new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh, pointCloudTopic, 1));
    sync.reset(new message_filters::Synchronizer<MySyncPolicy>(MySyncPolicy(10), *subCloudInfo,
                                                               *subLaserCloudOriginal));
    sync->registerCallback(&mapOptimization::laserCloudInfoHandler, this);
    ...
    if (savePCD) {
      ...
      save_velo_dir = savePCDDirectory + "velodyne/";
      unused = system((std::string("mkdir -p ") + save_velo_dir).c_str());

      pose_file.open(savePCDDirectory + "poses.txt");
      timestamp_file.open(savePCDDirectory + "times.txt");
    }
  }
  ...
  void allocateMemory() {
    ...
    laserCloudLast.reset(new pcl::PointCloud<PointXYZIRT>());
  ...
  void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &cloud_info_msg,
    ...
    pcl::fromROSMsg(*laser_cloud_msg, *laserCloudLast);
  ...
  pcl::PointCloud<PointType>::Ptr transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn,
    ...
    Eigen::Affine3f transCur =
        pcl::getTransformation(transformIn->x, transformIn->y, transformIn->z, transformIn->roll,
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  Eigen::Affine3f trans2Affine3f(float transformIn[]) {
    return pcl::getTransformation(transformIn[3], transformIn[4], transformIn[5], transformIn[0],
  ...
  void WriteRS80LidarDataToBinFile(const pcl::PointCloud<PointXYZIRT> &cloud_in,
  ...
  void SaveKeyframe(const pcl::PointCloud<PointXYZIRT> &rs_cloud) {
    static int file_id = 0;

    // save lidar
    std::stringstream ss;
    ss << std::setw(6) << std::setfill('0') << file_id++;
    const std::string scan_filename = ss.str() + ".bin";
    const std::string scan_filepath = save_velo_dir + scan_filename;

    WriteRS80LidarDataToBinFile(rs_cloud, scan_filepath);
    cout << scan_filename << endl;
  }

  void SaveKeypose(const PointTypePose &pose) {
    /*
    r11 r12 r13 tx
    r21 r22 r23 ty
    r31 r32 r33 tz
    ...
    in each line, we write as the following format
    r11 r12 r13 tx r21 r22 r23 ty r31 r32 r33 tz
    */

    tf::Matrix3x3 mat;
    mat.setRPY(pose.roll, pose.pitch, pose.yaw);
    // clang-format off
    pose_file << mat.getRow(0).x() << " " << mat.getRow(0).y() << " " << mat.getRow(0).z() << " " << pose.x << " "
              << mat.getRow(1).x() << " " << mat.getRow(1).y() << " " << mat.getRow(1).z() << " " << pose.y << " "
              << mat.getRow(2).x() << " " << mat.getRow(2).y() << " " << mat.getRow(2).z() << " " << pose.z << std::endl;
    // clang-format on

    // save absolute timestamp. << std::setprecision(6)
    ros::Time t;
    t.fromSec(pose.time);
    timestamp_file << t.toNSec() << std::endl;
  }
  ...
  void visualizeGlobalMapThread() {
    ...
    // laserCloudKeyFrames and cloud6D

    cout << "****************************************************" << endl;
    cout << "Saving map files, total num: " << laserCloudKeyFrames.size() << endl;
    // savePCDDirectory = std::getenv("HOME") + savePCDDirectory;

    for (int i = 0; i < (int)cloudKeyPoses6D->size(); i++) {
      SaveKeypose(cloudKeyPoses6D->points[i]);
    }

    // TODO(silin): do not save the followings
    ...
    *globalMapCloud += *globalSurfCloud;
    ...
    cout << "****************************************************" << endl;
    cout << "Saving map files completed" << endl;
  }
  ...
  void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key,
    ...
    for (int i = -searchNum; i <= searchNum; ++i) {
      ...
      *nearKeyframes +=
          *transformPointCloud(surfCloudKeyFrames[keyNear], &copy_cloudKeyPoses6D->points[keyNear]);
    }
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  bool LMOptimization(int iterCount) {
    ...
    // coordinate transformation lidar <- camera      ---     camera <- lidar x = z ---     x = y y
    // = x                ---     y = z z = y                ---     z = x roll = yaw           ---
    // roll = pitch pitch = roll         ---     pitch = yaw yaw = pitch          ---     yaw = roll
  ...
  void addGPSFactor() {
    ...
    while (!gpsQueue.empty()) {
      if (gpsQueue.front().header.stamp.toSec() < timeLaserInfoCur - 0.2) {
        ...
        gtsam::GPSFactor gps_factor(cloudKeyPoses3D->size(), gtsam::Point3(gps_x, gps_y, gps_z),
                                    gps_noise);
  ...
  void saveKeyFramesAndFactor() {
    ...
    pcl::PointCloud<PointXYZIRT>::Ptr thisKeyFrame(new pcl::PointCloud<PointXYZIRT>());
    pcl::copyPointCloud(*laserCloudCornerLastDS, *thisCornerKeyFrame);
    pcl::copyPointCloud(*laserCloudSurfLastDS, *thisSurfKeyFrame);
    pcl::copyPointCloud(*laserCloudLast, *thisKeyFrame);

    // save key frame cloud
    cornerCloudKeyFrames.push_back(thisCornerKeyFrame);
    surfCloudKeyFrames.push_back(thisSurfKeyFrame);
    laserCloudKeyFrames.push_back(thisKeyFrame);

    if (savePCD) {
      SaveKeyframe(*thisKeyFrame);
    }
